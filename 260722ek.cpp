...
void PrintKinematics(const std::string& label, mycobot::MyCobotDirect& robot, sensor::ImuState& imu) {
    ...
    double enc_x = ARM_LENGTH * sin(tilt_rad) * cos(pan_rad);
    double enc_y = ARM_LENGTH * sin(tilt_rad) * sin(pan_rad);
    double enc_z = (ARM_LENGTH * cos(tilt_rad)) + BASE_HEIGHT;
...
int main() {
    ...
    // 2. Initialize IMU multiplexer with the discovered IMU port
    ...
    // 3. Connect to the robot arm using the discovered arm port
    ...
    TestPosition(robot, imu_arm, {0, 45, 0, 0, 0, 0}, "Tilt 45");
    TestPosition(robot, imu_arm, {45, 0, 0, 0, 0, 0}, "Pan 45, Tilt 0");
    TestPosition(robot, imu_arm, {0, 90, 0, 0, 0, 0}, "Pan 0, Tilt 90");
