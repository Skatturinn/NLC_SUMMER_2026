...
int main() {
    ...
    //     }
    // }
    // 5. Variance Measurement Loop
    std::cout << "Moving to test position...\n";
    
    // Choose a pose where the arm is under some load (e.g., stretched out)
    mycobot::Angles test_target = {45.0, 45.0, 0.0, 0.0, 0.0, 0.0}; 
    robot_thread.QueueMovement(test_target, 20);
    
    // Give the arm time to reach the target and mechanically settle
    std::this_thread::sleep_for(std::chrono::milliseconds(4000));

    std::cout << "Recording encoder data for variance analysis...\n";
    std::ofstream csv_file("encoder_variance.csv");
    csv_file << "timestamp_ms,j1,j2,j3,j4,j5,j6\n";

    // Open CSV for IMU
    std::ofstream imu_csv("imu_variance.csv");
    imu_csv << "timestamp_ms,q_w,q_x,q_y,q_z\n";

    auto start_record = std::chrono::steady_clock::now();
    const int duration_ms = 10000; // Record for 10 seconds

    mycobot::Angles last_recorded = { -999, -999, -999, -999, -999, -999 };
  //  mycobot::Angles t_tar = {90.0, 90.0, 0.0, 0.0, 0.0, 0.0};
//    robot_thread.QueueMovement(t_tar, 20);
    while (true) {
        auto now = std::chrono::steady_clock::now();
        auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(now - start_record).count();

        if (elapsed > duration_ms) break;

        RobotState state = robot_thread.GetState();
        std::array<double, 4> q_imu = imu_arm.GetAlignedQuaternion();
        // Check if the 20Hz background thread has pulled new data since our last loop
//        if (state.encoders[0] != last_recorded[0] || state.encoders[1] != last_recorded[1] || state.encoders[2] != last_recorded[2]) {
        ...
        imu_csv << std::fixed << std::setprecision(6)
                ...
                << q_imu[2] << ","
                << q_imu[3] << "\n";
        last_recorded = state.encoders;
  //      }

        //10ms for imu, encoders should update every 50ms
        std::this_thread::sleep_for(std::chrono::milliseconds(10));
    }
    imu_csv.close();
    csv_file.close();
    std::cout << "Finished recording. Saved to encoder_variance.csv\n";
